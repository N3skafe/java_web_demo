...
public class BlogController {
    ...
    public String board_view(Model model, @PathVariable Long id, HttpSession session) { // , @RequestParam int orderNumber 삭제 오류남
    ...
    public String board_edit(Model model, @PathVariable Long id) { 
        ...
        if (list.isPresent()) {
            model.addAttribute("board", list.get()); // 존재하면Board 객체를모델에추가
        } else {
            // 처리할로직추가(예: 오류페이지로리다이렉트, 예외처리등)
            return "/error_page/article_error"; // 오류 처리 페이지로 연결(이름 수정됨)
        }
    ...
    public String updataBoard(@PathVariable Long id, @ModelAttribute AddArticleRequest request, HttpSession session) {
        ...
        Board board = list.get(); // 여기서 실제 Board 꺼내기
    ...
    public String addboards(@ModelAttribute AddArticleRequest request, HttpSession session) {
        String email = (String) session.getAttribute("email"); // 세션에서 이름 확인
        System.out.println("작성자: " + email);
        request.setUser(email);  // 또는 getNickname()
